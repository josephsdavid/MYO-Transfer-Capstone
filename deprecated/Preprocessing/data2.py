...
def format_data_to_train(vector_to_format):
    ...
    dim_c = (8, len(vector_to_format)//8)
    example = vector_to_format.reshape(dim_c)
    while (len(example) >= number_of_vector_per_example):
        # example = example.transpose()
        dataset_example_formatted.append(example.T)
        example = example[size_non_overlap:]
